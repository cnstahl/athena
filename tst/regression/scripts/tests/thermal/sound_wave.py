...
def prepare():
  ...
  #     python configure.py --prob=linear_wave
  #     
...
def run():
  ...
  arguments = [
      ...
      'problem/nuiso=0.002','problem/eta_O=0.0']
...
def analyze():
  ...
  ksqr = (2.0*np.pi)**2
  rate = ((gamma -1)**2/gamma* chi + 4/3 * nu)*ksqr/2 #(nu+eta)*k^2/2 decay rate of Alfven wave
